...
@Component({
  selector: 'app-ywjrw',
  templateUrl: './ywjrw.component.html',
  styleUrls: ['./ywjrw.component.css']
})
export class YwjrwComponent implements OnInit {
  data: Array<any> = [];
  pageIndex: number = 1;
  limit: number = 6;
  status: string; //
  ...
  getData(reset: boolean = false) {
    ...
    setTimeout(() => {
      ...
      this.CfService.getTaskgetend(this.pageIndex, this.limit, this.status, starttime, endtime, this.title).subscribe((json) => {
