...
@Component({
  selector: 'app-xjxt',
  templateUrl: './xjxt.component.html',
  styleUrls: ['./xjxt.component.css']
})
export class xjxtComponent implements OnInit {
  //数据
  all: Array<any> = [];
  channel: Array<any> = []; //
  userall: Array<any> = [];
  ...
  resetForm() {
    ...
    this.form.controls.assignor.setValue(this.userall);
    this.form.controls.isInterview.setValue(true);
    this.form.controls.open.setValue(true);
  ...
  getData(reset: boolean = false) {
    ...
    setTimeout(() => {
      this.XtService.getAll(this.pageIndex, this.limit, this.searchText, startTime, endTime, this.searchChannelid, this.searchStatus).subscribe((json) => {
  ...
  getMzkImg(item) {
    ...
    data && (this.mzkLoading = true) && this.MzkService.getMzkImg("searchassets", this.apik, data).subscribe(json => {
      const imgdata = json.DATA.map(item =>
        (json.COLUMNS.reduce((accumulator, currentValue, index) => ((accumulator[currentValue] = item[index]), accumulator), { check: false }))
      )
  ...
  getXtBind() {
    ...
    if (mediaids) {
      this.XtService.getAddRecord(this.recordId, mediaids).subscribe(json => {
        if (json.code === 0) {
          ...
          // this.taskClose();
          // this.resetForm();
          this.XtService.getInfo(this.recordId).subscribe(json => {
            if (json.code === 0) {
  ...
  removeFile() {
    ...
    file2 && this.MzkService.getDelXtmz(this.recordId, file2).subscribe(json => {
      console.log(json);
      if (json.code === 0) {
