...
@Component({
  selector: 'app-rwrl',
  templateUrl: './rwrl.component.html',
  styleUrls: ['./rwrl.component.css']
})
export class RwrlComponent implements OnInit {
  data: Array<any> = [];
  pageIndex: number = 1;
  limit: number = 6;
  status: string; //
  ...
  getData(reset: boolean = false) {
    ...
    setTimeout(() => {
      ...
      this.CfService.getOpen(this.pageIndex, this.limit, this.status, starttime, endtime, this.title).subscribe((json) => {
