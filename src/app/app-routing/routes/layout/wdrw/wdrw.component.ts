...
export class WdrwComponent implements OnInit {
  ...
  getTaskfinish(data) {
    ...
    this.modalService.create({
      ...
      nzClosable: false,
      nzOnOk: () => this.CfService.getTaskfinish(data.taskId, this.finishText).subscribe(json => {
        if (json.code === 0) {
          data.status = 2;
          this.message.info(json.msg);
        } else {
  ...
  getTaskexeaute() {
    if (this.taskText === "") {
      ...
      this.CfService.getTaskexeaute(this.taskId, this.taskText, mediaids).subscribe(json => {
        if (json.code === 0) {
          this.message.info(json.msg);
  ...
  constructor(private ngxXml2jsonService: NgxXml2jsonService, private MzkService: MzkService, private XtService: XtService, private fb: FormBuilder, private modalService: NzModalService, private datePipe: DatePipe, private CfService: CfService, private message: NzMessageService) {
    this.zxc.subscribe(console.log);
    this.zxc.next(123123123);
  ...
  mzkUpload(ev) {
    ...
    (ev.target.files.length > 0) && (this.isSpinning = true) && this.MzkService.getMzkUpload("c.apiupload", this.apik, this.mzkFloor[this.mzkFloor.length - 1].folder_id, ev.target.files[0]).subscribe(text => {
